...
public class CronJobServiceView extends VerticalLayout {
    ...
    public CronJobServiceView(CronJobLogStreamManager cronJobLogStreamManager, CronJobRestService cronJobRestService) {
    ...
    private void updateLogGrid() {
        recentLogs = cronJobLogStreamManager.getLogs().logs();
        logGrid.getDataProvider().refreshAll();
    }
    ...
    private void startLiveStreaming() throws URISyntaxException {
        ...
        cronJobLogStreamManager.addLogListener(logListener);

        // Get initial logs from the stream
        recentLogs.addAll(cronJobLogStreamManager.getLogs().logs());
        updateLogGrid();
    }
    ...
    private void stopLiveStreaming() {
        ...
        setupViewLiveButton();
        if (logListener != null) {
            cronJobLogStreamManager.removeLogListener(logListener);
            logListener = null;
        }
    }
